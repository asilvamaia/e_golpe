...
FEEDBACK_FILE = PASTA_DADOS / "feedbacks.json"
DB_AMEACAS_FILE = PASTA_DADOS / "database_ameacas.json"
...
def carregar_listas_seguranca():
    ...
    if not WHITELIST_FILE.exists():
        try:
            ...
            with open(WHITELIST_FILE, "w") as f: f.write("\n".join(WHITELIST_DEFAULT))
            whitelist = WHITELIST_DEFAULT
        except: whitelist = WHITELIST_DEFAULT
    else:
        try:
            with open(WHITELIST_FILE, "r") as f: whitelist = [line.strip().lower() for line in f if line.strip()]
        except: whitelist = WHITELIST_DEFAULT

    blacklist = []
    if BLACKLIST_FILE.exists():
        try:
            with open(BLACKLIST_FILE, "r") as f: blacklist = [line.strip().lower() for line in f if line.strip()]
        except: pass
...
def registrar_usuario(user_id):
    ...
    if USUARIOS_FILE.exists():
        try:
            with open(USUARIOS_FILE, "r") as f: lista = json.load(f)
        except: pass
    if user_id not in lista:
        lista.append(user_id)
        USUARIOS_FILE.parent.mkdir(parents=True, exist_ok=True)
        with open(USUARIOS_FILE, "w") as f: json.dump(lista, f)
        registrar_log(f"Novo usuário registrado: {user_id}", "INFO")
    ...
    if FEEDBACK_FILE.exists():
        try:
            with open(FEEDBACK_FILE, "r", encoding="utf-8") as f:
                lista_feedbacks = json.load(f)
        except: pass
    lista_feedbacks.append(entrada)
    FEEDBACK_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(FEEDBACK_FILE, "w", encoding="utf-8") as f:
        json.dump(lista_feedbacks, f, ensure_ascii=False, indent=2)
    registrar_log(f"Novo feedback recebido: {avaliacao}", "INFO")
...
def checar_virustotal(url):
    if not VIRUSTOTAL_KEY: return "Não Verificado (Falta API)"
    try:
        # VirusTotal v3 requer codificação Base64 sem padding '='
...
def atualizar_bases_ameacas():
    ...
    urls_unificadas = set()
    agora = time.time()
    try: DB_AMEACAS_FILE.parent.mkdir(parents=True, exist_ok=True)
    except: pass

    if DB_AMEACAS_FILE.exists():
        stat = DB_AMEACAS_FILE.stat()
        if agora - stat.st_mtime < 7200:
            if not CACHE_AMEACAS:
                try:
                    with open(DB_AMEACAS_FILE, 'r', encoding='utf-8') as f:
                        lista = json.load(f)
                        CACHE_AMEACAS = set(lista)
                    registrar_log(f"Cache de ameaças carregado: {len(CACHE_AMEACAS)} URLs", "INFO")
                except: pass
            return
    ...
    if urls_unificadas:
        ...
        try:
            with open(DB_AMEACAS_FILE, 'w', encoding='utf-8') as f:
                json.dump(list(urls_unificadas), f)
            registrar_log(f"Base de ameaças atualizada: {len(CACHE_AMEACAS)} URLs", "INFO")
        except Exception as e: registrar_log(f"Erro ao salvar DB: {e}", "ERRO")
    else:
        if not DB_AMEACAS_FILE.exists():
            with open(DB_AMEACAS_FILE, 'w') as f: f.write("[]")

threading.Thread(target=atualizar_bases_ameacas, daemon=True).start()

def checar_bases_phishing(url):
    if not CACHE_AMEACAS and DB_AMEACAS_FILE.exists():
        try:
            with open(DB_AMEACAS_FILE, 'r', encoding='utf-8') as f:
                lista = json.load(f)
                for u in lista: CACHE_AMEACAS.add(u)
        except: pass
...
def analisar_com_ia(url, dados_completos, origem="streamlit", metadados=None):
