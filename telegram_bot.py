...
def configurar_ia():
    ...
    for m in modelos:
        try:
            ...
            registrar_log(f"Falha IA {m}: {e}", "ALERTA")
            continue
    registrar_log("IA não conectada.", "ERRO")
...
def registrar_usuario(user_id):
    ...
    try:
        if not db.query(Usuario).filter(Usuario.user_id == str(user_id)).first():
            ...
            db.commit()
            bot.send_message(user_id, "🔔 *Bem-vindo ao GuardianBot!*\n\nVocê agora está protegido. Envie qualquer link suspeito ou mensagem duvidosa e eu analisarei para você!", parse_mode='Markdown')
...
def salvar_no_dataset(dados_entrada, analise_ia, metadados=None):
    ...
    try:
        novo_item = DatasetItem(
            ...
            metadados=metadados or {"origem": "Telegram"}
        )
        db.add(novo_item)
        db.commit()
    except Exception as e:
        registrar_log(f"Erro ao salvar dataset no DB (Telegram): {e}", "ERRO")
    finally:
        db.close()
...
def salvar_feedback(texto_usuario, resposta_ia, avaliacao):
    ...
    try:
        ...
        db.commit()
        registrar_log(f"Novo feedback Telegram recebido: {avaliacao}", "INFO")
    except Exception as e:
        registrar_log(f"Erro ao salvar feedback Telegram: {e}", "ERRO")
    finally:
        db.close()
...
def validar_seguranca_url(url):
    try:
        ...
        if not hostname or hostname in FORBIDDEN_HOSTS: return False, "Endereço inválido.", url
        
        # Tentativa 1: Hostname original
        try: 
            infos = socket.getaddrinfo(hostname, None)
            return True, None, url
        except socket.gaierror:
            # Tentativa 2: Tenta adicionar WWW se não tiver
...
def baixar_conteudo_seguro(url):
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'}
    try:
        response = requests.get(url, headers=headers, timeout=REQUEST_TIMEOUT, stream=True)
        return response.raw.read(MAX_CONTENT_SIZE + 1, decode_content=True).decode('utf-8', errors='ignore'), response.url
    except Exception as e: return None, str(e)

def analisar_conteudo_site(url):
    if not url.startswith("http"): url = "http://" + url
    html_content, info_extra = baixar_conteudo_seguro(url)
...
def checar_virustotal(url): return "Não Verificado" if not VIRUSTOTAL_KEY else "Sem Detecções"

def checar_urlscan(url):
    if not URLSCAN_KEY: return "Não Verificado (Falta API)"
    domain = limpar_dominio(url)
    try:
        headers = {'API-Key': URLSCAN_KEY, 'Content-Type': 'application/json'}
        params = {'q': f'domain:{domain}', 'size': 1}
        r = requests.get("https://urlscan.io/api/v1/search/", headers=headers, params=params, timeout=5)
        data = r.json()
        if data.get('results'):
...
def atualizar_bases_ameacas():
    ...
    if urls_unificadas:
        ...
        try:
            ...
            batch = [{"domain": d} for d in CACHE_AMEACAS]
...
def calcular_score_risco(dados, domain_clean):
    ...
    for bad in blacklist:
        if bad in domain_clean: score -= 100; fatores.append("BLACKLIST MANUAL: Domínio bloqueado pelo administrador (-100)")
...
def montar_instrucoes_formato(origem, score, classificacao, fatores_risco, alerta_sensivel=None):
    ...
    if origem == "streamlit":
        ...
        emoji = "🟢"
        if cor_vermelha: emoji = "🔴"
        elif classificacao in ["ATENÇÃO", "ALERTA"]: emoji = "🟡"
...
def consultar_google_reputacao(dominio):
...
# MODIFICAÇÃO: Inserir 'input' no dicionário antes de salvar
def analisar_com_ia(url, dados_completos, origem="telegram", metadados=None):
    registrar_log(f"Analisando URL: {url}", "INFO")
    
    # SALVA O INPUT PARA O HISTÓRICO
    dados_completos['input'] = url 
    
    if not MODELO_IA: return "⚠️ IA Offline."
    ...
    aviso_fakenews = None 
    info_reputacao = consultar_google_reputacao(domain_clean)
...
# MODIFICAÇÃO: Salva como 'input' em vez de 'texto'
def analisar_texto_ia(texto, origem="telegram", metadados=None):
...
if bot:
    ...
    def pedir_doacao(message):
        bot.reply_to(message, "☕ **Apoie o GuardianBot!**\n\nAjude a manter o projeto vivo: https://livepix.gg/SEU_PERFIL", parse_mode="Markdown")
    ...
    def processar_mensagem(message):
        ...
        if url:
            ...
            if not seguro_tecnico:
                bot.reply_to(message, f"🚫 **Bloqueado:** {motivo}")
                return
            
            url_final = url_validada

            try:
                # Importa diretamente de core
                from core import orquestrar_coleta_dados_url, analisar_com_ia, analisar_texto_ia, checar_cache_analise
                import asyncio
            ...
            from core import analisar_texto_ia, checar_cache_analise
            import asyncio
    ...
    def processar_arquivo(message):
        ...
        try:
            ...
            from core import analisar_arquivo_ia
            import asyncio
